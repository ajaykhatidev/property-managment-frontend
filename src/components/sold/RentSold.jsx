...
export const RentSold = () => {
  ...
  // Filter only Rent + Sold properties
  const rentSoldProperties =
    data?.filter(
      (property) =>
        property.rentOrSale === "Rent" && property.status === "Sold"
    ) || [];
  ...
  const filteredProperties = useMemo(() => {
    ...
    return rentSoldProperties.filter((property) => {
      ...
      const minPrice =
        searchFilters.minPrice === "" ? 0 : Number(searchFilters.minPrice);
      const maxPrice =
        searchFilters.maxPrice === "" ? Infinity : Number(searchFilters.maxPrice);
      ...
      const matchesOwnership =
        ...
        property.hpOrFreehold
          ?.toLowerCase()
          .includes(searchFilters.ownership.toLowerCase());

      return matchesText && matchesPrice && matchesBHK && matchesOwnership;
    });
  }, [data, searchFilters, rentSoldProperties]);
        ...
        <ul className="properties-grid">
          {filteredProperties.map((property) => (
            <li key={property._id} className="property-card">
              <div className="status-badge">Sold</div>
              ...
              <div className="property-location">
                <div className="property-info">
                  <p>
                    <strong>House No:</strong> {property.houseNo}
                  </p>
                  {property.block && (
                    <p>
